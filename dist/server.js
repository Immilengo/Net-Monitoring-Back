...
// src/config/env.ts
var import_dotenv, import_zod, envSchema, normalizedEnv, parsed, env, databaseUrl;
var init_env = __esm({
  "src/config/env.ts"() {
    "use strict";
    ...
    envSchema = import_zod.z.object({
      ...
      MONITOR_CRON_EXPRESSION: import_zod.z.string().default("*/2 * * * *"),
      MONITOR_ENABLED: import_zod.z.string().transform((v) => v === "true").default(true)
    });
    normalizedEnv = {
      ...process.env,
      MAIL_USERNAME: process.env.MAIL_USERNAME ?? process.env.MAIL_USER ?? ""
    };
    parsed = envSchema.safeParse(normalizedEnv);
    if (!parsed.success) {
      throw new Error(`Environment validation error: ${parsed.error.message}`);
    }
    env = parsed.data;
    databaseUrl = `postgresql://${env.DB_USER}:${env.DB_PASSWORD}@${env.DB_HOST}:${env.DB_PORT}/${env.DB_NAME}?schema=public`;
  }
});
...
var createUserSchema = import_zod3.z.object({
  ...
  roleName: import_zod3.z.string().min(1).optional()
});
...
var createSiteSchema = import_zod8.z.object({
  name: import_zod8.z.string().min(2, "Name must have at least 2 characters"),
  address: import_zod8.z.string().optional(),
  city: import_zod8.z.string().optional(),
  country: import_zod8.z.string().optional(),
  description: import_zod8.z.string().optional()
});
var updateSiteSchema = import_zod8.z.object({
  name: import_zod8.z.string().min(2).optional(),
  address: import_zod8.z.string().optional(),
  city: import_zod8.z.string().optional(),
  country: import_zod8.z.string().optional(),
  description: import_zod8.z.string().optional()
...
var siteRoutes = (0, import_express7.Router)();
...
var DeviceService = class {
  ...
  async create(input, actorId) {
    ...
    const device = await this.repository.create({
      ...
      type: input.type,
      description: input.description ?? null,
      currentStatus: import_client5.MonitoringStatus.OFFLINE,
      statusSource: import_client5.StatusSource.AUTO,
      active: true,
  ...
  async update(id, input, actorId) {
    ...
    if (input.siteId) await this.assertSiteExists(input.siteId);
    const statusSource = input.statusSource ?? (input.currentStatus !== void 0 ? import_client5.StatusSource.MANUAL : void 0);
...
var deviceTypeValues = Object.values(import_client6.DeviceType);
var monitoringStatusValues = Object.values(import_client6.MonitoringStatus);
var statusSourceValues = Object.values(import_client6.StatusSource);
var ipv4Regex = /^(\d{1,3}\.){3}\d{1,3}$/;
var createDeviceSchema = import_zod9.z.object({
  name: import_zod9.z.string().min(2, "Name must have at least 2 characters"),
  hostname: import_zod9.z.string().optional(),
  ipAddress: import_zod9.z.string().regex(ipv4Regex, "Invalid IPv4 address"),
  macAddress: import_zod9.z.string().optional(),
  type: import_zod9.z.enum(deviceTypeValues, {
    errorMap: () => ({ message: `Type must be one of: ${deviceTypeValues.join(", ")}` })
  }),
  description: import_zod9.z.string().optional(),
  siteId: import_zod9.z.string().uuid("Invalid site ID").optional()
});
var updateDeviceSchema = import_zod9.z.object({
  name: import_zod9.z.string().min(2).optional(),
  hostname: import_zod9.z.string().optional(),
  ipAddress: import_zod9.z.string().regex(ipv4Regex, "Invalid IPv4 address").optional(),
  macAddress: import_zod9.z.string().optional(),
  type: import_zod9.z.enum(deviceTypeValues).optional(),
  description: import_zod9.z.string().optional(),
  siteId: import_zod9.z.string().uuid().optional().nullable(),
  active: import_zod9.z.boolean().optional(),
  currentStatus: import_zod9.z.enum(monitoringStatusValues).optional(),
  statusSource: import_zod9.z.enum(statusSourceValues).optional()
}).refine((obj) => Object.keys(obj).length > 0, {
  message: "At least one field must be sent"
});
var listDeviceSchema = import_zod9.z.object({
  page: import_zod9.z.coerce.number().int().min(0).optional(),
  size: import_zod9.z.coerce.number().int().min(1).max(100).optional(),
  sortBy: import_zod9.z.enum(["name", "ipAddress", "type", "currentStatus", "createdAt", "updatedAt"]).optional(),
  direction: import_zod9.z.enum(["asc", "desc"]).optional(),
  recordStatus: import_zod9.z.enum(["ACTIVE", "INACTIVE", "ALL"]).optional(),
  search: import_zod9.z.string().optional(),
  siteId: import_zod9.z.string().uuid().optional(),
  type: import_zod9.z.enum(deviceTypeValues).optional(),
  currentStatus: import_zod9.z.enum(monitoringStatusValues).optional(),
  active: import_zod9.z.coerce.boolean().optional()
});

// src/modules/devices/routes/device.routes.ts
var controller7 = new DeviceController();
var deviceRoutes = (0, import_express8.Router)();
...
var serviceTypeValues = Object.values(import_client7.ServiceType);
var createServiceMonitorSchema = import_zod10.z.object({
  deviceId: import_zod10.z.string().uuid("Invalid device ID"),
  name: import_zod10.z.string().min(2, "Name must have at least 2 characters"),
  type: import_zod10.z.enum(serviceTypeValues, {
    errorMap: () => ({ message: `Type must be one of: ${serviceTypeValues.join(", ")}` })
  }),
  port: import_zod10.z.number({ invalid_type_error: "Port must be a number" }).int("Port must be an integer").min(1, "Port must be between 1 and 65535").max(65535, "Port must be between 1 and 65535"),
  enabled: import_zod10.z.boolean().optional().default(true),
  timeoutSeconds: import_zod10.z.number().int().min(1, "Timeout must be at least 1 second").max(60, "Timeout cannot exceed 60 seconds").optional().default(5)
});
var updateServiceMonitorSchema = import_zod10.z.object({
  name: import_zod10.z.string().min(2).optional(),
  type: import_zod10.z.enum(serviceTypeValues).optional(),
  port: import_zod10.z.number().int().min(1).max(65535).optional(),
  enabled: import_zod10.z.boolean().optional(),
  timeoutSeconds: import_zod10.z.number().int().min(1).max(60).optional()
}).refine((obj) => Object.keys(obj).length > 0, {
  message: "At least one field must be sent"
});
var listServiceMonitorSchema = import_zod10.z.object({
  page: import_zod10.z.coerce.number().int().min(0).optional(),
  size: import_zod10.z.coerce.number().int().min(1).max(100).optional(),
  sortBy: import_zod10.z.enum(["name", "type", "port", "createdAt", "updatedAt"]).optional(),
  direction: import_zod10.z.enum(["asc", "desc"]).optional(),
  deviceId: import_zod10.z.string().uuid().optional(),
  type: import_zod10.z.enum(serviceTypeValues).optional(),
  enabled: import_zod10.z.coerce.boolean().optional(),
  search: import_zod10.z.string().optional()
});

// src/modules/service-monitors/routes/service-monitor.routes.ts
var controller8 = new ServiceMonitorController();
var serviceMonitorRoutes = (0, import_express9.Router)();
...
var AlertService = class {
  ...
  async getSummary() {
    const [total, critical, warning, info] = await Promise.all([
      ...
      this.repository.countByLevel(import_client8.AlertLevel.CRITICAL),
      this.repository.countByLevel(import_client8.AlertLevel.WARNING),
      this.repository.countByLevel(import_client8.AlertLevel.INFO)
    ]);
...
var alertLevelValues = Object.values(import_client9.AlertLevel);
var listAlertSchema = import_zod11.z.object({
  page: import_zod11.z.coerce.number().int().min(0).optional(),
  size: import_zod11.z.coerce.number().int().min(1).max(100).optional(),
  sortBy: import_zod11.z.enum(["level", "createdAt", "updatedAt"]).optional(),
  direction: import_zod11.z.enum(["asc", "desc"]).optional(),
  deviceId: import_zod11.z.string().uuid().optional(),
  level: import_zod11.z.enum(alertLevelValues).optional(),
  acknowledged: import_zod11.z.coerce.boolean().optional(),
  resolved: import_zod11.z.coerce.boolean().optional(),
  search: import_zod11.z.string().optional()
});
var acknowledgeAlertSchema = import_zod11.z.object({
  note: import_zod11.z.string().max(500).optional()
});

// src/modules/alerts/routes/alert.routes.ts
var controller9 = new AlertController();
var alertRoutes = (0, import_express10.Router)();
...
var DashboardRepository = class {
  ...
  async getDeviceStatusCounts() {
    const [total, online, offline, warning] = await Promise.all([
      ...
      prisma.device.count({ where: { deleted: false, currentStatus: import_client10.MonitoringStatus.ONLINE } }),
      prisma.device.count({ where: { deleted: false, currentStatus: import_client10.MonitoringStatus.OFFLINE } }),
      prisma.device.count({ where: { deleted: false, currentStatus: import_client10.MonitoringStatus.WARNING } })
    ]);
  ...
  async getTopUnreachable(limit = 5) {
    ...
    const results = await prisma.monitoringLog.groupBy({
      by: ["deviceId"],
      where: {
        status: import_client10.MonitoringStatus.OFFLINE,
        checkedAt: { gte: since }
      },
      _count: { id: true },
      orderBy: { _count: { id: "desc" } },
...
var dashboardRoutes = (0, import_express11.Router)();
...
var monitoringStatusValues2 = Object.values(import_client11.MonitoringStatus);
var listMonitoringLogSchema = import_zod12.z.object({
  page: import_zod12.z.coerce.number().int().min(0).optional(),
  size: import_zod12.z.coerce.number().int().min(1).max(200).optional(),
  sortBy: import_zod12.z.enum(["checkedAt", "status", "responseTime"]).optional(),
  direction: import_zod12.z.enum(["asc", "desc"]).optional(),
  deviceId: import_zod12.z.string().uuid().optional(),
  siteId: import_zod12.z.string().uuid().optional(),
  status: import_zod12.z.enum(monitoringStatusValues2).optional(),
  from: import_zod12.z.coerce.date().optional(),
  to: import_zod12.z.coerce.date().optional(),
  search: import_zod12.z.string().optional()
});
var statsQuerySchema = import_zod12.z.object({
  deviceId: import_zod12.z.string().uuid("Invalid device ID"),
  from: import_zod12.z.coerce.date().optional(),
  to: import_zod12.z.coerce.date().optional()
});
var latestByDeviceSchema = import_zod12.z.object({
  limit: import_zod12.z.coerce.number().int().min(1).max(200).optional()
});

// src/modules/monitoring-logs/routes/monitoring-log.routes.ts
var controller11 = new MonitoringLogController();
var monitoringLogRoutes = (0, import_express12.Router)();
...
// src/shared/router.ts
var router = (0, import_express13.Router)();
...
init_prisma();
var passwordHash = async (password) => import_bcrypt4.default.hash(password, 10);
var seedDemoDataIfNeeded = async () => {
  ...
  const device = await prisma.device.upsert({
    ...
    update: {
      name: "Core Router",
      type: import_client12.DeviceType.ROUTER,
      currentStatus: import_client12.MonitoringStatus.ONLINE,
      statusSource: import_client12.StatusSource.AUTO,
      siteId: site.id,
      active: true,
      deleted: false
    },
  ...
  await prisma.monitoringLog.create({
    data: {
      deviceId: device.id,
      status: import_client12.MonitoringStatus.ONLINE,
      responseTime: 12,
      packetLoss: 0,
  ...
  await prisma.alert.create({
    data: {
      ...
      level: import_client12.AlertLevel.INFO
    }
  });
  await prisma.serviceMonitor.create({
    data: {
      deviceId: device.id,
      name: "ICMP Ping",
      type: import_client12.ServiceType.TCP,
      port: 0,
      enabled: true,
      timeoutSeconds: 5
    }
...
var icmpCheck = (host, timeoutSeconds = 5) => {
  return new Promise((resolve) => {
    ...
    const start = Date.now();
    (0, import_child_process.exec)(command, { timeout: (timeoutSeconds + 2) * 1e3 }, (error, stdout) => {
...
var AlertEngineService = class {
  ...
  async handleDeviceStatusChange(device, newStatus) {
    try {
      if (newStatus === import_client13.MonitoringStatus.OFFLINE) {
        await prisma.alert.create({
          data: {
            deviceId: device.id,
      ...
      if (newStatus === import_client13.MonitoringStatus.ONLINE) {
...
var MonitoringCheckService = class {
  ...
  async checkDevice(device) {
    try {
      ...
      const newStatus = result.reachable ? import_client14.MonitoringStatus.ONLINE : import_client14.MonitoringStatus.OFFLINE;
      ...
      const isManualOverride = device.statusSource === import_client14.StatusSource.MANUAL;
      if (isManualOverride) {
...
var startMonitoringScheduler = () => {
  ...
  schedulerTask = import_node_cron.default.schedule(expression, async () => {
    if (isRunning) {
